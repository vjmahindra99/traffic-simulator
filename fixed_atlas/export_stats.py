...
def export_stats_to_xlsx(filename="fixed_time_stats.xlsx"):
    ...
    queue_at_green_sum = settings.queue_at_green_sum
    ...
    wb = Workbook()
    ws = wb.active
    ws.title = "Fixed-time Stats"

    # Title
    title_text = "Fixed-time Traffic Light Statistics"
    ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=5)
    title_cell = ws.cell(row=1, column=1, value=title_text)

    title_cell.font = Font(size=14, bold=True, color="FFFFFF")
    title_cell.alignment = Alignment(horizontal="center", vertical="center")
    title_cell.fill = PatternFill(start_color="FF0000", end_color="FF0000", fill_type="solid") # red colour
    ...
    # Save workbook
    wb.save(filepath)
    print("[Fixed-time] stats exported to", filepath)